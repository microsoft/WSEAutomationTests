...
$maps = RetrieveMap
$videoResMap = $maps["VideoResMap"]
$photoResMap = $maps["PhotoResMap"]
...
{  
   ...
   {  
      ...
      $vdoResDetails = $videoResMap[$vdoRes]
      ...
      {   
         ...
         $ptoResDetails = $photoResMap[$ptoRes]
            ...
            while ($true) {
               ...
               if ($token -and $SPId -and ($runMode -eq "Both" -or [string]::IsNullOrEmpty($runMode))) {
                  if ($batteryPercentage -lt 20) {
                     while ($batteryPercentage -lt 80 -and $pluggedInLast -lt $deviceData["ToggleAiEffect"].Count - 1) {
                        ...
                        CameraAppTest -logFile "CameraAppTest.txt" -token $token -SPId $SPId -initSetUpDone $initialSetupDone -camsnario $camsnario -VF $VF -vdoRes $vdoResDetails -ptoRes $ptoResDetails -devPowStat $devPowStat -toggleEachAiEffect $togAiEfft >> "$pathLogsFolder\CameraAppTest.txt"
                        $batteryPercentage = Get-BatteryPercentage
                     }
                  } else {
               ...
               elseif ($runMode -eq "PluggedInOnly" -or ([string]::IsNullOrEmpty($runMode) -and $chargingState -eq "Charging")) {
                  if ($pluggedInLast -lt $deviceData["ToggleAiEffect"].Count - 1) {
                     ...
                     $togAiEfft = $deviceData["ToggleAiEffect"][$pluggedInLast]
                     $devPowStat = "PluggedIn"
                     CameraAppTest -logFile "CameraAppTest.txt" -initSetUpDone $initialSetupDone -camsnario $camsnario -VF $VF -vdoRes $vdoResDetails -ptoRes $ptoResDetails -devPowStat $devPowStat -toggleEachAiEffect $togAiEfft >> "$pathLogsFolder\CameraAppTest.txt"
                  } else {
...
write-Host "pathLogsFolder is $pathLogsFolder"
