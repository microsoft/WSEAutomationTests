...
<#
DESCRIPTION:
    This function automates testing of the Windows Settings App, specifically focusing on camera AI effects for External USB Camera.
    It applies various AI effect configurations, collects performance traces, captures CPU and NPU usage, 
    and verifies if logs are generated correctly.
INPUT PARAMETERS:
    - devPowStat [string] :- The power state of the device (e.g., "PluggedIn", "OnBattery").
...
RETURN TYPE:
    - void
#>
function SettingAppTest-Playlist_External_Camera($devPowStat, $testScenario, $token, $SPId) 
{   
   try
   { 
       $startTime = Get-Date    
       $ErrorActionPreference='Stop'
       $scenarioName = "$devPowStat\$testScenario"
       $logFile = "$devPowStat-SettingAppTest_External_Camera.txt"
       $resourceUtilizationFile = "$pathLogsFolder\$devPowStat-resource_utilization.txt"
       $resourceUtilizationConsolidated = "$pathLogsFolder\$devPowStat-consolidate_stats.txt"
       #Check device Power state
       $devState = CheckDevicePowerState $devPowStat $token $SPId
