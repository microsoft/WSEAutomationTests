...
function InitializeTest($TstsetNme, $targetMepCameraVer, $targetMepAudioVer, $targetPerceptionCoreVer)
{
    ...
    $Global:Results = '' | SELECT ScenarioName,fps,TotalNumberOfFrames,FramesAbove33ms,'AvgProcessingTimePerFrame(In ms)','MaxProcessingTimePerFrame(In ms)','MinProcessingTimePerFrame(In ms)','timetofirstframe(In secs)','CameraAppInItTime(In secs)','VoiceRecorderInItTime(In secs)','timetofirstframeForAudio(In secs)',FramesAbove33msForAudioBlur,'PeakWorkingSetSize(In MB)','AvgWorkingSetSize(In MB)', MedianCPUUsage, MedianNPUUsage, MedianMemoryUsage, PeakCPUUsage, PeakNPUUsage, PeakMemoryUsage, AverageCPUUsage, AverageNPUUsage, AverageMemoryUsage, Status,ReasonForNotPass
