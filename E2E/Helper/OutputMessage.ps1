...
function AddToFailedTestsList($failedTests)
{
   ...
   $token = "111222"
   $SPID ="333444"
   $powerProfile = $splitEachTests[7]  # Added logic to handle power profile

   if($functionToCall -eq  "CameraAppTest")
   {
      Write-Output "$functionToCall -logFile $logFile $token $SPId -camsnario $camsnario -vdoRes $vdoRes -ptoRes $ptoRes -devPowStat $devPowStat -VF $VF -toggleEachAiEffect $togAiEfft -powerProfile $powerProfile >> `$pathLogsFolder\CameraAppTest.txt" >> $pathLogsFolder\ReRunFailedTests.ps1
