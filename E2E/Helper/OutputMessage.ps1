...
function ResetFields {
   ...
   $Results.Status = $null
   $Results.ReasonForNotPass = $null
   $Results.'AvgMemoryUsage(In GB)'= $null
   $Results.'AvgNPUUsage(In %)'= $null
   $Results.'AvgCPUUsage(In %)'= $null
   $Results.'BeforeMemoryUsage(In GB)'= $null
   $Results.'BeforeNPUUsage(In %)'= $null
   $Results.'BeforeCPUUsage(In %)'= $null
}
   ...
   if (-not [string]::IsNullOrWhiteSpace($textFileContent))
   {
       ...
       foreach ($textData in ($textFileContent -split "`r`n`r`n" | Where-Object { $_.Trim() -ne '' }))
       {
          ...
          $propertyValuePairs | ForEach-Object{
              $name = $_.Property
              if ($name -eq 'SessionName') { $name = 'PC Version' }
              $excelRow | Add-Member -MemberType NoteProperty -Name $name -Value $_.Value
          }
